...
class bw_segmentizer(gr.top_block, Qt.QWidget):
    ...
    def __init__(self):
        ...
        self._sweep_rate_range = qtgui.Range(1, int( samp_rate / (n_channels - 1) ), 1, 1, 200)
        self._sweep_rate_win = qtgui.RangeWidget(self._sweep_rate_range, self.set_sweep_rate, "sweep_rate", "counter_slider", float)
        ...
        # List of all stream_to_vec, comp_to_mag², thresholds
        all_stream_to_vec = []
        all_cmpl_to_mag2 = []
        all_thresholds = []

        # Block to sweep across frequencies
        f_sweeper = epy_block_0_0.blk(n_channels=n_channels, samp_rate=samp_rate, sweep_rate=sweep_rate, f_start=f_start, f_stop=f_stop)
        ...
        sig_source = analog.sig_source_c(samp_rate, analog.GR_SIN_WAVE, f_start, 1, 0, 0)

        ##################################################
        # Connections
        ##################################################
        self.connect((sig_source, 0), (throttle, 0))
        self.connect((throttle, 0), (f_sweeper, 0))
        self.connect((throttle, 0), (self.PFB_channelizer, 0))
        self.msg_connect((f_sweeper, "frequency"), (sig_source, "cmd"))
        for i in range(n_channels):
